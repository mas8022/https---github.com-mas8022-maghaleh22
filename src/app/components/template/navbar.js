...
export default function Navbar() {
  ...
  const [haveMe, setHaveMe] = useState("");

  useEffect(() => {
    fetch("/api/isMe")
      .then((res) => res.json())
      .then((data) => setHaveMe(data));
    ...
    <div className="fixed top-0 left-0 w-full h-[5rem] md:h-[8rem] shadow-md flex items-center justify-between px-6 sm:px-[5rem] md:px-[10rem] xl:px-[15rem] bg-white z-[1000]">
      ...
      <div className="flex gap-4 sm:gap-8 md:gap-10 items-center">
        {!haveMe ? (
          ...
          <Link
            href={"login"}
            className="w-[10rem] md:w-[12rem] h-[3rem] md:h-[4rem] text-[1.3rem] md:text-[1.7rem] rounded-md bg-slate-500/30 active:bg-slate-500/20 flex items-center justify-center cursor-pointer active:text-emerald-600"
          >
            ورود/ثبت نام
          </Link>
        )}
